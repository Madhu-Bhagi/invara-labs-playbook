# First Paying Client — 90-Day Execution Plan

**Version:** 1.0.0  
**Status:** Approved  
**Owner:** Madhukumar Rajanala  
**Start Date:** 31 July 2026  
**Target Date:** 29 October 2026

## Goal

Win Invara Labs' first paying client by turning the company's engineering expertise and existing intellectual property into a credible, easy-to-buy consulting offer.

This plan preserves the approved positioning: **engineering-first, AI-enabled, and business-focused**. AI is a differentiating capability, not the company's primary identity.

## North Star

Every planned activity must do at least one of the following:

1. Help win the next client.
2. Strengthen Invara Labs' reputation.
3. Create a reusable asset with clear commercial value.

Work that does none of these is deferred.

## Success Criteria

The goal is achieved when Invara Labs has a signed, paid client engagement. The supporting launch baseline is:

- A professional website is live.
- Three consulting packages have a defined buyer, outcome, scope, duration, and price model.
- The EEOS repository is publishable and supports sales conversations.
- One reference implementation demonstrates delivery capability.
- One flagship reusable accelerator has a credible first release.
- A qualified prospect list and active outreach cadence exist.

## Business Pillars

| Pillar | Current role | Near-term focus |
|---|---|---|
| Consulting | Revenue engine | Architecture reviews, Angular and Nx modernisation, AI adoption, EEOS implementation, and engineering governance |
| Products | Reusable delivery leverage | Angular Enterprise Starter Kit and reusable delivery assets |
| Education | Trust and demand generation | Weekly technical content and public engineering knowledge |
| Platform | Long-term proprietary capability | Validate demand before building AI-powered engineering tools |

Consulting is the immediate priority. Products and education support consulting. Platform investment begins only after customer evidence justifies it.

## 90-Day Outcomes

### Days 1–30 — Make the offer sellable

- Select an ideal customer profile and a primary buyer.
- Define three outcome-based consulting packages.
- Create the website's positioning, services, proof, and contact content.
- Audit the EEOS repository for public-release blockers; fix only material credibility issues.
- Build a qualified prospect list and begin direct outreach.

**Exit evidence:** service one-pagers, website copy, repository release checklist, prospect list, and recorded outreach.

### Days 31–60 — Establish proof and demand

- Launch the minimum credible website.
- Publish one useful technical article each week.
- Publish or demonstrate one reference implementation.
- Conduct discovery conversations and record objections, needs, and buying signals.
- Refine packages using market feedback.

**Exit evidence:** live website, four published pieces, reference implementation, discovery notes, and revised offers.

### Days 61–90 — Convert interest into revenue

- Release the minimum credible Angular Enterprise Starter Kit.
- Continue targeted outreach and follow-up.
- Run architecture or AI-adoption discovery sessions with qualified prospects.
- Issue proposals with explicit outcomes, scope, timeline, and commercial terms.
- Close and onboard the first paying client.

**Exit evidence:** accelerator release, opportunity pipeline, proposals, and a signed paid engagement.

## Weekly Operating Cadence

| Activity | Minimum cadence |
|---|---|
| Qualified outreach | 10 new prospects per week |
| Follow-up | Twice weekly |
| Discovery conversations | 2 per week once pipeline permits |
| Public technical content | 1 useful publication per week |
| Pipeline and priority review | Weekly |
| Reusable asset work | Only after weekly revenue work is complete |

## Decision Rules

- Start outreach before every supporting asset is perfect.
- Prefer a paid diagnostic or focused engagement over a broad custom proposal.
- Build reusable assets from observed client needs, not speculation.
- Treat the EEOS as commercial proof; do not add more playbooks during this plan.
- Do not build the long-term platform until repeated customer demand is demonstrated.
- The founder remains accountable for company decisions; AI provides analysis and recommendations, not authority.

## Deferred Work

- Additional playbooks or governance frameworks.
- A comprehensive engineering maturity model.
- Multiple accelerators at the same time.
- A proprietary AI platform without validated demand.
- Documentation work that does not remove a publication or sales blocker.

## Review

Progress is reviewed weekly against leading indicators—outreach, conversations, qualified opportunities, and proposals—and the outcome indicator: signed revenue. This plan is reviewed formally on 29 October 2026 or immediately after the first client is won.

## Related Documents

- [Founder Vision](../01-foundation/01-founder-vision.md)
- [Mission](../01-foundation/02-mission.md)
- [Vision](../01-foundation/03-vision.md)
- [Core Values](../01-foundation/04-core-values.md)
- [Repository Roadmap](../../ROADMAP.md)

## Revision History

| Version | Date | Change | Status |
|---|---|---|---|
| 1.0.0 | 31 July 2026 | Established the first-client goal and 90-day company execution plan | Approved |
